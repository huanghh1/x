...
export function buildYokaiResearchPrompt(symbol, intervalCode) {
  ...
  return [
    ...
    "核心检查维度：筹码集中：Top10 holder 占比、Top holder 类型、是否需要排除 CEX 热钱包、LP、staking、bridge、项目锁仓或做市地址。",
    "检查市值看看是否为小市值的代币",
    "OI/MCap：合约 OI value 与市值或流通市值的比例；OI/MCap > 3x 属于高风险信号。",
    "Vol/OI：成交额相对 OI 的异常程度；Vol/OI > 20x 时要警惕刷量或高频对倒。",
    "资金费率陷阱：持续深度负费率 < -0.05% 且价格抗跌/OI 上升，偏诱空或挤压蓄力；费率转正后要观察是否出货。",
    "订单簿结构：Bid-Ask 是否失衡、Ask 是否变薄、拉盘前上方卖压是否被快速吃掉；没有盘口数据就写缺失。",
    "wallet -> CEX：项目方、早期大户或异常钱包是否向 CEX 转入；没有地址标签或转账证据就写缺失。",
    "价格结构：是否处在区间底部、横盘吸筹、挤压蓄力、快速拉高、急跌出货或双向收割阶段。",
    "",
    "评分权重：筹码集中 25 分，资金费率异常 20 分，OI/MCap 异常 15 分，Vol/OI 刷量嫌疑 15 分，价格接近区间底部或挤压蓄力位置 10 分，Bundler/机器人/同源钱包 10 分，订单簿结构 5 分，总分 0-100。操盘模式识别：挤压式：建仓 -> 诱饵拉盘 -> 深度负费率引空 -> 挤压爆空 -> 反手做空。",
